...
function Table(tbl)
  ...
  -- Use more aggressive default to maximize page width utilization
  local max_table_width = 0.98  -- More aggressive default for better content display
  if PANDOC_STATE.meta and PANDOC_STATE.meta.max_table_width then
    ...
    if type(meta_value) == "table" and meta_value.t == "MetaInlines" then
      ...
      max_table_width = tonumber(pandoc.utils.stringify(meta_value)) or 0.98
    elseif type(meta_value) == "number" then
      max_table_width = meta_value
    elseif type(meta_value) == "string" then
      max_table_width = tonumber(meta_value) or 0.98
    end
  end
  
  -- Apply conservative width reduction only for very wide tables
  -- Allow better utilization of page width for better content display
  if num_cols >= 6 then
    max_table_width = max_table_width * 0.96  -- 4% reduction for very wide tables
  elseif num_cols >= 5 then
    max_table_width = max_table_width * 0.98  -- 2% reduction for wide tables
  end
